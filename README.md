# 灵兽"白泽" (Baize) - Obsidian 插件架构设计

## 1. 核心定位

"白泽"在中国传统文化中有通晓万物的特质。本插件旨在成为个人知识库中"无所不知"的灵兽，通过语义检索和本地智能，打破传统关键词搜索的局限，建立一个隐私、极速、深度的知识助手。

### 📸 UI 预览

**桌面端** — 右侧面板常驻对话 / 语义搜索 / 灵感联想

![桌面端 UI 原型](UI%20原型/desktop.png)

**移动端** — 底部抽屉 + 单手操作优化

![移动端 UI 原型](UI%20原型/mobile.png)

---

## 2. 技术栈

| 层级       | 技术选型                                       | 职责                           |
| ---------- | ---------------------------------------------- | ------------------------------ |
| 向量数据库 | LanceDB (WASM)                                 | 本地高性能持久化向量存储与检索 |
| 向量化模型 | `transformers.js` (ONNX Runtime WASM)          | 本地文本向量化，隐私不出本地   |
| LLM 大脑   | 云端: DeepSeek / OpenAI API; 本地: Ollama 接口 | RAG 检索增强生成               |
| 宿主环境   | Obsidian (Electron 桌面 / Capacitor 移动端)    | 插件运行时                     |
| UI 框架    | Svelte 5                                       | 响应式视图层                   |
| 构建工具   | esbuild                                        | 产物打包与 WASM 资源处理       |

---

## 3. 核心功能模块

### 3.1 语义搜索 (Semantic Search)
使用自然语言意图搜索，而非死板的关键词匹配。用户输入一段话，白泽返回语义最相关的笔记片段。

### 3.2 白泽对话屏 (Chat View)
侧边栏对话框，基于库内知识回答问题并附带引用来源。采用 RAG 流程：查询 → 语义检索上下文 → 拼接 Prompt → 流式回复。

### 3.3 知识联想 (Insight)
自动根据当前打开的笔记内容，推荐库中语义最相关的其他笔记片段，实现"被动发现"。

### 3.4 实时索引 (Live Indexing)
自动监听 Vault 中的文件变更（`create`、`modify`、`delete`、`rename`），秒级维护向量库，无需手动触发全量扫描。

---

## 4. 分层架构（Layered Architecture）

整体采用**四层分层架构 + 跨切面服务**，自底向上职责层层递进，每层只依赖其下层，严禁跳层依赖。

```text
┌──────────────────────────────────────────────────────┐
│                  UI 层 (Presentation)                 │
│           Svelte 组件 · 平台适配视图入口              │
├──────────────────────────────────────────────────────┤
│                 应用层 (Application)                  │
│     用例编排 · 索引调度 · RAG 管线 · 同步服务        │
├──────────────────────────────────────────────────────┤
│                  领域层 (Domain)                      │
│    分块策略 · 向量化抽象 · LLM 客户端 · 数据模型     │
├──────────────────────────────────────────────────────┤
│               基础设施层 (Infrastructure)             │
│     LanceDB 适配器 · 模型管理 · Worker 池 · 平台适配 │
└──────────────────────────────────────────────────────┘
  ↕ 跨切面 (Cross-cutting): 类型系统 · 事件总线 · 日志 · 配置管理
```

### 4.1 依赖规则
- UI 层 → 应用层 → 领域层 → 基础设施层
- 领域层定义**接口 (Interface)**，基础设施层提供**实现 (Implementation)**（依赖反转原则）
- 跨切面服务可被任意层引用

---

## 5. 项目代码结构

```text
baize-obsidian-plugin/
├── src/
│   ├── main.ts                    # 插件入口：生命周期管理、注册命令/视图/事件
│   │
│   ├── infrastructure/            # ═══ 基础设施层 ═══
│   │   ├── database/              # 向量数据库实现
│   │   │   ├── lance-adapter.ts   #   LanceDB WASM 适配器 (实现 IVectorStore)
│   │   │   └── schema.ts         #   向量表结构声明
│   │   ├── models/                # 模型与资源管理
│   │   │   ├── model-manager.ts   #   模型下载、量化选择、缓存路径、生命周期
│   │   │   ├── onnx-embedder.ts   #   transformers.js 封装 (实现 IEmbedder)
│   │   │   └── llm-clients/       #   各 LLM 提供商的具体实现
│   │   │       ├── deepseek.ts    #     DeepSeek API 客户端
│   │   │       ├── openai.ts      #     OpenAI API 客户端
│   │   │       └── ollama.ts      #     Ollama 本地客户端
│   │   ├── workers/               # Web Worker 并行计算
│   │   │   ├── embedding.worker.ts #   独立线程：模型加载 + 推理 (解决 UI 卡顿)
│   │   │   └── worker-pool.ts     #   Worker 线程池管理器
│   │   ├── storage/                # 统一存储
│   │   │   └── vault-storage.ts   #   封装 Obsidian DataAdapter (桌面/移动端统一)
│   │   └── platform/              # 跨平台适配
│   │       ├── platform-detect.ts #   运行时平台检测 (Desktop / Android / iOS)
│   │       ├── desktop.ts         #   桌面端能力 (拖拽、全局快捷键)
│   │       ├── android.ts         #   Android 特有逻辑 (返回键、后台任务、功耗控制)
│   │       └── ios.ts             #   iOS 特有逻辑 (安全区域、后台限制、触控反馈)
│   │
│   ├── domain/                    # ═══ 领域层 ═══
│   │   ├── interfaces/            # 核心抽象接口 (纯类型，无实现)
│   │   │   ├── vector-store.ts    #   IVectorStore: 向量存取接口
│   │   │   ├── embedder.ts        #   IEmbedder: 文本 → 向量接口
│   │   │   └── llm-provider.ts    #   ILLMProvider: LLM 调用接口
│   │   ├── models/                # 领域数据模型
│   │   │   ├── document-chunk.ts  #   文档分块的核心数据结构
│   │   │   └── search-result.ts   #   搜索结果数据结构
│   │   └── chunking/              # 文本分块策略
│   │       ├── markdown-chunker.ts #   Markdown 感知的分块器
│   │       └── strategies.ts      #   分块策略 (固定长度 / 标题分割 / 语义分段)
│   │
│   ├── application/               # ═══ 应用层 ═══
│   │   ├── index-scheduler.ts     #   索引调度中心：全量扫描 + 增量同步编排
│   │   ├── sync-service.ts        #   实时文件变更监听 → 增量索引
│   │   ├── search-service.ts      #   语义搜索服务：查询编排
│   │   ├── rag-pipeline.ts        #   RAG 管线：检索 → 上下文拼接 → LLM 调用
│   │   └── insight-service.ts     #   知识联想服务：基于当前笔记推荐相关片段
│   │
│   ├── ui/                        # ═══ UI 层 ═══
│   │   ├── views/                 # Obsidian ItemView 注册入口
│   │   │   ├── sidebar-view.ts    #   桌面端：右侧面板视图容器
│   │   │   └── modal-view.ts      #   移动端：全屏/半屏 Modal 容器
│   │   ├── components/            # Svelte 共享组件
│   │   │   ├── SearchBar.svelte   #   语义搜索输入框
│   │   │   ├── ChatPanel.svelte   #   对话面板 (消息列表 + 输入)
│   │   │   ├── ResultCard.svelte  #   搜索结果 / 联想卡片
│   │   │   ├── InsightPanel.svelte #  知识联想面板
│   │   │   └── StatusBar.svelte   #   索引进度 / 模型状态指示器
│   │   └── layouts/               # 平台差异化布局
│   │       ├── DesktopLayout.svelte #  桌面布局：侧边栏 Tab 切换
│   │       └── MobileLayout.svelte  #  移动布局：底部抽屉 + 单手优化
│   │
│   ├── shared/                    # ═══ 跨切面 ═══
│   │   ├── types.ts               #   全局类型定义
│   │   ├── constants.ts           #   常量 (插件ID、默认配置、缓存路径)
│   │   ├── event-bus.ts           #   全局事件总线 (解耦层间通信)
│   │   ├── logger.ts              #   统一日志服务 (dev/prod 级别控制)
│   │   └── errors.ts              #   自定义错误类型与错误处理策略
│   │
│   └── settings/                  # 设置管理
│       ├── settings-tab.ts        #   Obsidian 设置面板 UI
│       └── default-settings.ts    #   默认配置值与配置类型定义
│
├── styles/                        # CSS 样式 (构建时合并为 styles.css)
│   ├── variables.css              #   全局 CSS 变量 (色彩、字体、阴影、间距)
│   ├── base.css                   #   基础重置与公共样式
│   ├── components.css             #   组件样式 (搜索框、卡片、对话气泡)
│   ├── desktop.css                #   桌面专属 (侧边面板、分屏、拖拽反馈)
│   └── mobile.css                 #   移动专属 (底部抽屉、单手操作、触控热区)
│
├── build/                         # 构建输出 (main.js, styles.css, manifest.json)
├── manifest.json                  # Obsidian 插件清单
├── esbuild.config.mjs             # 构建脚本
├── package.json                   # 依赖管理
└── tsconfig.json                  # TypeScript 配置
```

---

## 6. 移动端适配策略 (Android / iOS)

### 6.1 模型优化
| 平台    | 模型精度    | 典型大小  | 内存占用 | 备注                     |
| ------- | ----------- | --------- | -------- | ------------------------ |
| 桌面端  | FP32 / FP16 | ~90 MB    | ~200 MB  | 追求精度                 |
| Android | Q4 (4-bit)  | ~25 MB    | ~60 MB   | 设备内存差异大，需动态检测 |
| iOS     | Q4 (4-bit)  | ~25 MB    | ~60 MB   | 内存压力更严格，超限会被系统杀死 |

`model-manager.ts` 根据 `platform-detect.ts` 返回的平台类型自动选择最优精度。移动端强制使用量化模型，iOS 额外设置内存水位监控，逼近阈值时主动释放模型缓存。

### 6.2 统一存储方案

**不自建存储抽象，直接使用 Obsidian 原生 `DataAdapter`。**

Obsidian 的 `app.vault.adapter` 已经在内部实现了三端统一的文件 I/O：
- 桌面端 → Node.js `fs`
- Android → Capacitor Filesystem API → Android 原生文件系统
- iOS → Capacitor Filesystem API → iOS 沙箱文件系统

插件不需要关心底层差异，同一套代码在三个平台上直接运行。

```text
┌───────────────────────────────────────────────┐
│    上层消费者 (lance-adapter / model-manager)  │
│                     ↓                         │
│           vault-storage.ts (薄封装层)          │
│                     ↓                         │
│       Obsidian DataAdapter (平台内置)          │
│   read() · readBinary() · writeBinary()       │
│   exists() · remove() · list()                │
├─────────────┬─────────────────┬───────────────┤
│  Desktop   │  Android          │  iOS            │
│  Node fs   │  原生文件系统    │  沙箱文件系统  │
└─────────────┴─────────────────┴───────────────┘
          Obsidian 在启动时已完成平台适配
```

`vault-storage.ts` 是一个**薄封装层**，职责仅为：
- 将插件的相对路径拼接为完整 Vault 路径
- 统一错误处理与日志
- 提供类型安全的二进制读写方法

**存储路径约定**：
- **向量数据**：`.obsidian/plugins/baize-obsidian-plugin/data.lance/`
- **模型缓存**：`.obsidian/plugins/baize-obsidian-plugin/cache/`

> 为什么不用 IndexedDB 或 SQLite？因为白泽的持久化需求是**文件级 I/O**（读写 `.lance` 二进制文件和模型权重），不是关系型查询。Obsidian 的 `DataAdapter` 已经是最佳答案——零额外依赖，原生跨平台。

### 6.3 功耗控制

| 策略             | Android                                    | iOS                                      |
| ---------------- | ------------------------------------------ | ---------------------------------------- |
| 索引触发模式     | `自动` / `仅充电时` / `手动`              | `自动` / `仅充电时` / `手动`             |
| 后台执行         | 支持后台任务，可用 `requestIdleCallback` 分片 | iOS 后台严格限制，仅在前台时执行索引   |
| 内存水位监控     | 可选（Android 内存容忍度较高）             | 必须启用（超限即被系统杀死）           |
| 模型释放策略     | App 进入后台 30s 后释放                    | App 进入后台立即释放                  |

### 6.4 UI 差异化

**桌面端 (Desktop)**：
- 右侧面板常驻对话/搜索/联想三栏 Tab 切换
- 支持拖拽搜索结果片段至编辑器
- 全局快捷键 `Ctrl/Cmd+Shift+B` 呼出语义搜索

**Android**：
- 底部抽屉 (Bottom Sheet) 半屏/全屏滑出
- 单手操作优化，交互元素锁定在屏幕下半部
- 系统返回键兼容：返回键关闭抽屉而非退出插件
- 卡片式流式布局，针对小屏幕优化信息密度

**iOS**：
- 底部抽屉 (Bottom Sheet) 半屏/全屏滑出
- 单手操作优化，交互元素锁定在屏幕下半部
- Safe Area 适配：尊重刘海屏、灵动岛等安全区域，避免内容被遮挡
- 触控反馈：使用 iOS 原生风格的滑动惯性与回弹效果

---

## 7. 核心架构优化

### ⚡ 7.1 Web Worker 并行化

```text
┌─────────────────┐        postMessage         ┌──────────────────────┐
│    主线程        │  ──── 文本分块数据 ────▶   │  embedding.worker.ts │
│  (UI + 调度)     │                            │  (transformers.js)   │
│                  │  ◀── Float32Array 向量 ──  │  (ONNX Runtime WASM) │
└─────────────────┘        onmessage           └──────────────────────┘
```

- **问题**：transformers.js 模型推理属于 CPU 密集型任务，直接运行在主线程会导致 Obsidian 界面卡死。
- **方案**：所有模型加载与推理任务运行在 `embedding.worker.ts` 中。主线程仅通过 `postMessage` 发送文本，接收向量结果。
- **线程池**：`worker-pool.ts` 管理多个 Worker 实例（桌面端 2-4 个，移动端 1 个），支持任务排队和并行处理。
- **移动端注意**：Capacitor 环境中 Web Worker 无法直接调用 Capacitor 插件 API（缺少 `window` 对象），因此 Worker 内只做纯计算，所有平台 API 调用回到主线程。

### 📦 7.2 动态模型加载

- **策略**：插件安装包 (`main.js`) 不包含模型权重文件，保持轻量。
- **逻辑**：首次使用时，`model-manager.ts` 根据平台下载匹配的模型版本并缓存。
- **离线回退**：若无网络且无缓存，提示用户手动导入模型文件。

### 🔄 7.3 事件驱动解耦

```text
sync-service  ──▶  EventBus.emit('file:changed', payload)
                         │
          ┌──────────────┼──────────────┐
          ▼              ▼              ▼
   index-scheduler   insight-service   StatusBar (UI更新)
```

各层通过 `event-bus.ts` 通信，避免直接引用产生耦合。例如：
- `sync-service` 检测文件变更后发射事件，不直接调用索引器。
- `index-scheduler` 监听事件后调度 Worker 执行向量化。
- UI 组件监听事件更新进度状态。

### 🧪 7.4 构建优化

- **WASM 处理**：`esbuild.config.mjs` 中 `loader: { '.wasm': 'file' }` 确保 WASM 文件被拷贝而非内联。
- **CSS 合并**：构建时将 `styles/` 下所有 CSS 合并为一个 `styles.css` 输出。
- **Tree Shaking**：桌面端与移动端的平台特定代码通过运行时检测 (`Platform.isMobile`) 确保不会加载不需要的模块。

---

## 8. 数据流与关键流程

### 8.1 索引流程 (Indexing Pipeline)

```text
Vault 文件变更
     │
     ▼
sync-service (监听 Obsidian 事件)
     │
     ▼
index-scheduler (判断增量/全量，分块任务分发)
     │
     ▼
markdown-chunker (文本 → 语义分块)
     │
     ▼
worker-pool → embedding.worker (分块文本 → 向量)
     │
     ▼
lance-adapter (向量 + 元数据 → 写入 LanceDB)
```

### 8.2 搜索流程 (Search Pipeline)

```text
用户输入查询
     │
     ▼
search-service (查询预处理)
     │
     ▼
embedding.worker (查询文本 → 查询向量)
     │
     ▼
lance-adapter (ANN 最近邻搜索 → Top-K 结果)
     │
     ▼
ResultCard (UI 渲染搜索结果 + 点击跳转)
```

### 8.3 RAG 对话流程

```text
用户提问
     │
     ▼
search-service (语义检索 → Top-K 相关片段)
     │
     ▼
rag-pipeline (拼接上下文 Prompt)
     │
     ▼
llm-provider (调用 DeepSeek/OpenAI/Ollama → 流式回复)
     │
     ▼
ChatPanel (流式渲染回复 + 引用来源高亮)
```

---

## 9. 实施路线 (Roadmap)

---

### 第一阶段：项目骨架与基础设施

**1.1 项目初始化**
- [x] 使用 Obsidian 官方插件模板初始化项目结构
- [x] 配置 `tsconfig.json`：启用严格模式、ESNext target、路径别名 (`@/`)
- [x] 配置 `package.json`：添加 `dev`、`build`、`lint` 脚本
- [x] 配置 `.gitignore`：排除 `build/`、`node_modules/`、`cache/`
- [x] 编写 `manifest.json`：插件 ID、名称、版本、最低 Obsidian 版本要求

**1.2 构建系统 (`esbuild.config.mjs`)**
- [x] 配置 esbuild 基础 TypeScript → JS 编译
- [x] 配置 Svelte 插件 (`esbuild-svelte`)：支持 `.svelte` 文件编译
- [x] 配置 WASM loader：`{ '.wasm': 'file' }` 确保 WASM 文件拷贝而非内联
- [x] 配置 CSS 合并：`styles/` 下所有 CSS 合并为 `build/styles.css`
- [x] 配置 Web Worker 打包：Worker 文件独立打包为单独 JS
- [x] 配置 `watch` 模式：开发时自动重编译
- [x] 配置输出到 `build/` 目录：`main.js` + `styles.css` + `manifest.json`

**1.3 插件入口 (`main.ts`)**
- [x] 继承 `Plugin` 类，实现 `onload()` / `onunload()` 生命周期
- [x] 注册插件设置面板 (`addSettingTab`)
- [x] 注册侧边栏视图 (`registerView`)
- [x] 注册命令：呼出语义搜索 (`addCommand`)
- [x] 注册 Ribbon 图标按钮
- [x] 初始化事件总线 (`event-bus.ts`)
- [x] 初始化日志服务 (`logger.ts`)
- [x] 根据平台检测结果初始化平台适配模块
- [x] 启动索引调度器 (`index-scheduler.ts`)
- [x] `onunload()` 中释放 Worker 线程池、关闭 LanceDB 连接

**1.4 跨切面服务 (`shared/`)**
- [x] `types.ts`：定义全局类型（`PluginSettings`、`PlatformType`、`IndexStatus`）
- [x] `constants.ts`：插件 ID、缓存路径、默认设置值、支持的模型列表
- [x] `event-bus.ts`：实现简易 EventEmitter（`on`、`off`、`emit`、`once`）
- [x] `event-bus.ts`：定义事件类型枚举（`file:changed`、`index:progress`、`model:ready` 等）
- [x] `logger.ts`：分级日志（`debug`、`info`、`warn`、`error`），生产环境关闭 debug
- [x] `errors.ts`：自定义错误类（`ModelLoadError`、`IndexError`、`StorageError`、`LLMError`）

**1.5 平台检测与适配 (`infrastructure/platform/`)**
- [x] `platform-detect.ts`：通过 `Platform.isMobile`、`Platform.isIosApp`、`Platform.isAndroidApp` 检测平台
- [x] `platform-detect.ts`：导出 `getPlatform(): 'desktop' | 'android' | 'ios'` 函数
- [x] `desktop.ts`：注册全局快捷键 `Ctrl/Cmd+Shift+B` 呼出语义搜索
- [x] `desktop.ts`：实现拖拽支持（搜索结果 → 编辑器）
- [x] `android.ts`：注册系统返回键处理（关闭抽屉而非退出插件）
- [x] `android.ts`：实现后台任务分片逻辑 (`requestIdleCallback`)
- [x] `ios.ts`：Safe Area CSS 变量注入
- [x] `ios.ts`：App 进入后台时立即释放模型缓存

**1.6 统一存储 (`infrastructure/storage/`)**
- [x] `vault-storage.ts`：封装 `app.vault.adapter`，提供 `readBinary()` / `writeBinary()`
- [x] `vault-storage.ts`：实现插件相对路径 → 完整 Vault 路径的拼接
- [x] `vault-storage.ts`：实现 `ensureDir()` 自动创建缺失目录
- [x] `vault-storage.ts`：统一错误处理（文件不存在、权限不足、磁盘空间不足）
- [x] 验证存储在 Desktop / Android / iOS 三端的读写一致性

---

### 第二阶段：向量数据库与索引引擎

**2.1 LanceDB 集成 (`infrastructure/database/`)**
- [x] `schema.ts`：定义向量表结构（`id`、`file_path`、`chunk_index`、`text`、`vector`、`metadata`、`updated_at`）
- [x] `lance-adapter.ts`：实现 `IVectorStore` 接口
- [x] `lance-adapter.ts` → `init()`：初始化 LanceDB WASM 实例，打开/创建数据库
- [x] `lance-adapter.ts` → `upsert()`：插入或更新向量记录（按 `file_path + chunk_index` 去重）
- [x] `lance-adapter.ts` → `delete()`：按 `file_path` 删除该文件的所有向量记录
- [x] `lance-adapter.ts` → `search()`：ANN 近似最近邻查询，返回 Top-K 结果
- [x] `lance-adapter.ts` → `getStats()`：返回索引统计信息（总记录数、总文件数、数据库大小）
- [x] `lance-adapter.ts` → `close()`：安全关闭数据库连接
- [x] 验证 LanceDB WASM 在 Desktop / Android / iOS 三端能正常初始化与读写

**2.2 领域接口 (`domain/interfaces/`)**
- [x] `vector-store.ts`：定义 `IVectorStore` 接口（`init`、`upsert`、`delete`、`search`、`getStats`、`close`）
- [x] `embedder.ts`：定义 `IEmbedder` 接口（`loadModel`、`embed(text): Promise<Float32Array>`、`embedBatch`、`unloadModel`）
- [x] `llm-provider.ts`：定义 `ILLMProvider` 接口（`chat`、`chatStream`、`getModels`、`testConnection`）

**2.3 领域数据模型 (`domain/models/`)**
- [x] `baize-file.ts`：定义文件聚合根，包含文件元数据、索引状态管理及分块集合
- [x] `baize-chunk.ts`：定义分块模型，包含文本内容、位置信息（行号/偏移量）及元数据引用

**2.4 文本分块 (`domain/chunking/`)**
- [x] `strategies.ts`：定义分块策略枚举（`FIXED_LENGTH`、`HEADING_SPLIT`、`HYBRID`）
- [x] `markdown-chunker.ts`：实现 Markdown 标题级别感知分块（按 `#`、`##`、`###` 切分）
- [x] `markdown-chunker.ts`：处理代码块不被拆分（检测 ``` 边界）
- [x] `markdown-chunker.ts`：实现固定长度与混合分块（在句子/换行边界处优化切分）
- [x] `markdown-chunker.ts`：实现块大小限制与重叠度 (Overlap 15%)
- [x] `markdown-chunker.ts`：保留分块元数据（标题路径、起止行号、字符偏移量）
- [x] `markdown-chunker.ts`：自动忽略 YAML frontmatter 区域
- [x] 实现中文分块优化（支持 。！？ 等中文标点作为断句依据）

**2.5 Web Worker 通信 (`infrastructure/workers/`)**
- [x] `embedding.worker.ts`：Worker 内部初始化 `transformers.js` pipeline
- [x] `embedding.worker.ts`：监听 `postMessage` 接收文本分块数据
- [x] `embedding.worker.ts`：执行模型推理，通过 `postMessage` 返回 `Float32Array` 向量
- [x] `embedding.worker.ts`：支持批量推理（一次接收多个文本，批量返回向量）
- [x] `embedding.worker.ts`：实现模型加载进度回报（`model:loading` 事件 + 百分比）
- [x] `embedding.worker.ts`：实现 Worker 内错误捕获与回传
- [x] `worker-pool.ts`：实现线程池管理器（桌面端 2-4 Worker，移动端 1 Worker）
- [x] `worker-pool.ts`：实现任务队列（FIFO，待处理任务排队）
- [x] `worker-pool.ts`：实现 `submit(task): Promise<result>` 提交任务接口
- [x] `worker-pool.ts`：实现空闲 Worker 自动分配与回收
- [x] `worker-pool.ts`：实现 `terminate()` 安全销毁所有 Worker

**2.6 模型管理 (`infrastructure/models/`)**
- [x] `model-manager.ts`：定义模型元数据与描述，支持模型列表获取
- [x] `model-manager.ts`：实现平台敏感的模型推荐逻辑（Mobile 优先量化模型）
- [x] `model-manager.ts`：实现 `isCached()` 检查本地模型文件夹是否存在
- [x] `model-manager.ts`：从 HuggingFace CDN 下载模型权重（带进度回调与重试）
- [x] `model-manager.ts`：下载失败重试机制（最多 3 次，指数退避）
- [x] `model-manager.ts`：支持用户手动导入模型文件（离线场景）
- [x] `model-manager.ts`：模型缓存路径管理（`.obsidian/plugins/baize-obsidian-plugin/models/`）
- [x] `model-manager.ts`：实现 `getModelPath()` 返回当前运行所需的真实路径
- [x] `model-manager.ts`：实现 `clearCache()` 调用 Storage 彻底清理缓存
- [x] `onnx-embedder.ts`：基于 Web Worker 线程池封装 `transformers.js`
- [x] `onnx-embedder.ts`：实现 `IEmbedder` 领域接口
- [x] `onnx-embedder.ts`：实现向量归一化（L2 norm，已在 Worker 中集成）

**2.7 索引调度 (`application/`)**
- [x] `sync-service.ts`：注册 Obsidian Vault 事件监听（`create`、`modify`、`delete`、`rename`）
- [x] `sync-service.ts`：文件变更防抖（500ms 内多次修改合并为一次索引任务）
- [x] `sync-service.ts`：过滤非 Markdown 文件（忽略图片、PDF、附件等）
- [x] `sync-service.ts`：过滤用户排除的目录/文件（基于设置的 `excludePaths`）
- [x] `sync-service.ts`：通过事件总线发射 `file:changed` 事件
- [x] `index-scheduler.ts`：监听 `file:changed` 事件，调度增量索引任务
- [x] `index-scheduler.ts`：实现全量扫描逻辑（首次启动或用户手动触发时）
- [x] `index-scheduler.ts`：全量扫描进度通知（`index:progress` 事件 + 已处理/总数）
- [x] `index-scheduler.ts`：实现增量索引逻辑（仅处理变更的文件）
- [x] `index-scheduler.ts`：并发控制（通过 Worker Pool 队列实现）
- [x] `index-scheduler.ts`：移动端功耗模式感知（初步通过 isMobile 策略实现）
- [x] `index-scheduler.ts`：索引完成后发射 `index:complete` 事件

---

### 第三阶段：搜索、RAG 与知识联想

**3.1 语义搜索服务 (`application/search-service.ts`)**
- [x] 接收用户查询文本
- [x] 调用 Worker Pool 将查询文本向量化
- [x] 调用 `lance-adapter.search()` 执行 Top-K ANN 搜索
- [x] 对搜索结果按相似度分数排序
- [x] 过滤低于阈值（如 < 0.3）的结果
- [x] 对结果文本进行关键词高亮处理（辅助 UI 显示）
- [x] 返回 `SearchResult[]` 领域对象给 UI 层

**3.2 RAG 对话管线 (`application/rag-pipeline.ts`)**
- [x] 接收用户提问
- [x] 调用 `search-service` 检索 Top-K 相关片段
- [x] 拼接系统 Prompt（角色定义 + 回答规则 + 引用格式要求）
- [x] 拼接上下文 Prompt（将检索到的片段按格式注入）
- [x] 拼接用户 Prompt（用户原始提问）
- [x] 调用 `ILLMProvider.chatStream()` 获取流式回复
- [x] 解析回复中的引用标记思路（通过 Prompt 强制输出 [^index] 格式）
- [x] 将流式 token 推送给 UI 层实时渲染（通过 onChunk 回调）
- [x] 实现对话历史管理（最近 N 轮对话作为上下文窗口）
- [x] 实现 Token 计数器（简易字符启发式计数器，防止超出窗口）

**3.3 LLM 客户端 (`infrastructure/models/llm-clients/`)**
- [x] `deepseek.ts`：实现 `ILLMProvider` 接口，接入 DeepSeek API
- [x] `deepseek.ts`：支持流式响应 (SSE)
- [x] `openai.ts`：实现 `ILLMProvider` 接口，接入 OpenAI API
- [x] `openai.ts`：支持流式响应 (SSE)
- [x] `openai.ts`：兼容 OpenAI 格式的第三方 API（自定义 Base URL）
- [x] `ollama.ts`：实现 `ILLMProvider` 接口，接入本地 Ollama
- [x] `ollama.ts`：支持流式响应
- [x] `ollama.ts`：自动检测本地 Ollama 服务是否可用
- [x] 所有客户端实现 `testConnection()` 连接测试方法
- [x] 所有客户端实现请求超时处理（默认 30s）
- [x] 所有客户端实现错误重试（网络错误重试 2 次）

**3.4 知识联想服务 (`application/insight-service.ts`)**
- [x] 监听 Obsidian 活动笔记切换事件 (`active-leaf-change`)
- [x] 获取当前笔记全文内容
- [x] 对当前笔记内容进行分块并通过 `SearchService.searchByVector` 获取语义特征
- [x] 调用 `vectorStore.search()` 搜索最相关的其它笔记片段（排除当前笔记自身）
- [x] 防抖处理（用户快速切换笔记时，延迟 800ms 执行联想任务）
- [x] 缓存最近 N 篇笔记的联想结果，采用 LRU 策略
- [x] 通过事件总线推送 `insight:updated` 事件给 UI 层

---

### 第四阶段：UI 视图与交互

**4.1 桌面端视图 (`ui/views/sidebar-view.ts`)**
- [x] 继承 `ItemView`，注册视图类型 `baize-sidebar`
- [x] 实现 `onOpen()`：挂载 Svelte `DesktopLayout` 组件
- [x] 实现 `onClose()`：销毁 Svelte 组件实例
- [x] 实现 `getViewType()` / `getDisplayText()` / `getIcon()`

**4.2 移动端视图 (`ui/views/modal-view.ts`)**
- [x] 继承 `Modal`，实现全屏/半屏弹出
- [x] 实现 `onOpen()`：挂载 Svelte `MobileLayout` 组件
- [x] 实现 `onClose()`：销毁 Svelte 组件实例
- [x] Android：注册返回键关闭 Modal（已在 Modal 默认行为中涵盖）
- [x] iOS：适配 Safe Area（已在 Svelte 中实现）

**4.3 桌面布局 (`ui/layouts/DesktopLayout.svelte`)**
- [x] 实现三栏 Tab 切换（搜索 / 对话 / 联想）
- [x] Tab 切换动画（渐变过渡）
- [x] 记住用户上次激活的 Tab（通过 `BaizeSettings.lastActiveTab`）
- [x] 面板顶部显示索引状态指示器 (`StatusBar` 组件实时监听 EventBus)

**4.4 移动布局 (`ui/layouts/MobileLayout.svelte`)**
- [x] 实现底部抽屉交互样式（Bottom Sheet 布局与弹出动画）
- [x] 实现分段控件 (Segmented Control) 切换搜索/对话/联想
- [x] 所有交互元素锁定在屏幕下半部（便于单手操作）
- [x] iOS：适配 Safe Area（适配 Home Indicator 区域）
- [x] Android：适配系统导航栏高度（通过全局变量同步）

**4.5 搜索组件 (`ui/components/SearchBar.svelte`)**
- [x] 搜索输入框 + 搜索图标
- [x] 输入防抖（300ms 后自动触发搜索）
- [x] 搜索中状态显示（CSS spinner 加载动画）
- [x] 清空按钮（圆形 × 按钮）
- [x] 搜索历史记录（最近 10 条，localStorage 持久化，支持单条/全部删除）

**4.6 搜索结果卡片 (`ui/components/ResultCard.svelte`)**
- [x] 显示匹配片段文本（带关键词高亮，安全 HTML 渲染）
- [x] 显示来源笔记标题 + 标题层级路径
- [x] 显示相似度分数（带颜色等级：绿 > 金 > 橙 > 灰）
- [x] 点击跳转到原始笔记对应行号
- [x] 桌面端：支持拖拽卡片内容到编辑器
- [x] 复制按钮：一键复制片段内容（带"已复制"反馈）

**4.7 对话面板 (`ui/components/ChatPanel.svelte`)**
- [x] 消息列表渲染（用户消息 + AI 回复交替显示，气泡布局）
- [x] AI 回复流式逐字显示（打字机效果 + 闪烁光标）
- [x] AI 回复中的引用链接可点击（`[^N]` 格式，金色下划线样式）
- [x] AI 回复中的 Markdown 格式渲染（代码块、行内代码、粗体、斜体、引用、列表）
- [x] 底部输入框 + 发送按钮（金色主题）
- [x] 输入框支持 `Enter` 发送，`Shift+Enter` 换行
- [x] 对话清空按钮（新建对话，顶部工具栏）
- [x] 消息加载中状态（三点脉冲"白泽正在思考..."）
- [x] 停止生成按钮（中断流式回复）
- [x] 滚动行为：新消息自动滚到底部

**4.8 知识联想面板 (`ui/components/InsightPanel.svelte`)**
- [x] 显示当前笔记的相关片段推荐列表（带排名徽章，第 1 名金色高亮）
- [x] 每个推荐项显示来源笔记标题 + 标题层级 + 匹配片段预览 + 分数
- [x] 点击跳转到推荐笔记对应位置
- [x] 无结果时显示空状态提示（带图标和引导文字）
- [x] 索引未完成时显示引导提示（引导用户先完成索引）

**4.9 状态指示器 (`ui/components/StatusBar.svelte`)**
- [x] 显示索引进度（X / Y 文件已索引，带金色进度条）
- [x] 显示模型加载状态（未加载 / 加载中 / 就绪 / 错误，彩色状态点）
- [x] 显示模型下载进度（渐变进度条）
- [x] 显示数据库统计（总向量数、数据库大小）
- [x] 点击展开详细信息面板（含模型名称、平台信息、折叠动画）

---

### 第五阶段：设置与配置

**5.1 设置面板 (`settings/settings-tab.ts`)**
- [x] **模型设置**：选择 embedding 模型（预设模型列表下拉框）
- [x] **模型设置**：模型精度选择（自动 / FP16 / Q4）
- [x] **模型设置**：模型缓存管理（查看大小 + 清除缓存按钮）
- [x] **LLM 设置**：LLM 提供商选择（DeepSeek / OpenAI / Ollama / 自定义）
- [x] **LLM 设置**：API Key 输入框（密码类型，不明文显示）
- [x] **LLM 设置**：自定义 API Base URL 输入框
- [x] **LLM 设置**：对话模型选择（模型名称输入或下拉）
- [x] **LLM 设置**：连接测试按钮（验证 API Key 有效性）
- [x] **索引设置**：排除目录/文件列表（支持 glob 模式）
- [x] **索引设置**：分块策略选择（标题分割 / 固定长度 / 语义段落）
- [x] **索引设置**：分块大小设置（最大 tokens 数）
- [x] **索引设置**：移动端索引触发模式（自动 / 仅充电时 / 手动）
- [x] **索引设置**：手动触发全量重建索引按钮
- [x] **搜索设置**：默认返回结果数 (Top-K)
- [x] **搜索设置**：最低相似度阈值
- [x] **高级设置**：Worker 线程数（桌面端，默认自动）
- [x] **高级设置**：日志级别（debug / info / warn / error）
- [x] **关于**：插件版本、数据库统计、作者信息

**5.2 默认配置 (`settings/default-settings.ts`)**
- [x] 定义 `BaizeSettings` 类型接口
- [x] 定义所有设置项的默认值
- [x] 实现设置值校验函数（类型检查、范围限制）

---

### 第六阶段：样式与视觉 (`styles/`)

- [x] `variables.css`：定义色彩体系 CSS 变量（亮色/暗色主题双套）
- [x] `variables.css`：定义字体、字号、间距、圆角、阴影 CSS 变量
- [x] `base.css`：基础重置样式 + 滚动条自定义
- [x] `components.css`：SearchBar 组件样式
- [x] `components.css`：ResultCard 卡片样式（悬停效果、分数标签）
- [x] `components.css`：ChatPanel 对话气泡样式（用户/AI 差异化配色）
- [x] `components.css`：InsightPanel 推荐项样式
- [x] `components.css`：StatusBar 进度条 + 状态标签样式
- [x] `components.css`：Tab 切换 / Segmented Control 样式
- [x] `desktop.css`：侧边面板骨架布局
- [x] `desktop.css`：拖拽交互视觉反馈（拖拽中的半透明副本）
- [x] `mobile.css`：Bottom Sheet 滑动动画
- [x] `mobile.css`：单手操作热区高亮
- [x] `mobile.css`：iOS Safe Area padding
- [x] `mobile.css`：Android 导航栏适配
- [x] 验证 Obsidian 默认主题（亮色 + 暗色）下的视觉一致性
- [x] 验证主流第三方主题（Minimal、Things、Blue Topaz）的兼容性

---

### 第七阶段：测试与质量保证

- [ ] 单元测试：`markdown-chunker.ts` 各种分块策略
- [ ] 单元测试：`event-bus.ts` 事件订阅/发射/取消
- [ ] 单元测试：`vault-storage.ts` 路径拼接与错误处理
- [ ] 单元测试：`rag-pipeline.ts` Prompt 拼接逻辑
- [ ] 集成测试：Worker 通信（主线程 ↔ Worker 消息收发）
- [ ] 集成测试：完整索引流程（文件变更 → 分块 → 向量化 → 存储）
- [ ] 集成测试：完整搜索流程（查询 → 向量化 → ANN 搜索 → 返回结果）
- [ ] 性能测试：1000 篇笔记全量索引耗时基准
- [ ] 性能测试：10000 条向量 Top-10 搜索延迟基准
- [ ] 性能测试：移动端 (Android/iOS) 内存占用监控
- [ ] 边界测试：空 Vault（零文件）状态处理
- [ ] 边界测试：超大文件（> 100KB 单文件）分块验证
- [ ] 边界测试：网络断开时 LLM 调用错误恢复
- [ ] 边界测试：模型下载中断后的断点续传 / 重试

---

### 第八阶段：发布与运营

- [ ] 编写完整 README（功能截图、安装步骤、使用指南、配置说明）
- [ ] 录制功能演示 GIF / 视频
- [ ] 编写 CHANGELOG.md
- [ ] 编写 LICENSE（MIT）
- [ ] 配置 GitHub Actions CI（自动构建 + 发布 Release）
- [ ] 构建生产包验证（`main.js` 体积、WASM 文件完整性）
- [ ] 提交 Obsidian 社区插件仓库 PR（`obsidian-releases`）
- [ ] 社区插件审核反馈修复
- [ ] 首版发布 (v0.1.0)
- [ ] 收集用户反馈渠道搭建（GitHub Issues 模板）

---

*"知万物之情，通天下之理。"*